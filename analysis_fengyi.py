...
def quantify(sample_dir_path, classes, header, save_table, template_path):
    ...
    # 测量每个结构的体素数
    # 路径是 'sample_dir_path/volume/result.mhd'
    mhd_annotation_file = os.path.join(sample_dir_path, 'volume', 'result.mhd')
    
    try:
        img = sitk.ReadImage(mhd_annotation_file)
        I_mask = sitk.GetArrayFromImage(img)
    ...
    print(f"--- 调试信息 for {dataset_name} ---")
    print(f"I_mask 形状 (shape): {I_mask.shape}")
    print(f"I_mask 数据类型 (dtype): {I_mask.dtype}")
    min_val = np.min(I_mask) 
    max_val = np.max(I_mask) 
    print(f"I_mask 最小值: {min_val}") 
    print(f"I_mask 最大值: {max_val}")
    
    if min_val < 0:
        print(f"!!! 警告: 在图像中找到了负值。")
        # 打印出所有唯一值，这通常很有帮助
        unique_vals = np.unique(I_mask)
        print(f"图像中所有的唯一值: {unique_vals}")
    else:
        print("--- 图像中没有找到负值。 (这不应导致 bincount 错误)")

    I_mask_flat = I_mask.ravel()
    total_volume = np.sum(I_mask_flat > 0) * mm
    ...
    for i in range(1, len(ids)):
        current_id_path = f"/{ids[i]}/"
        idx = path.str.contains(current_id_path, na=False)
        df_new[i] = np.sum(df_volumes[idx])
        
    df_volumes = df_new
    df_header = f"{dataset_name}_Volume"
    table_volume = pd.DataFrame(df_volumes, columns=[df_header])
    df_vol = pd.concat([df_template.iloc[:, [0, -2, -1]], table_volume], axis=1)
    
    print(f"{dataset_name}\t Total structure volume: {total_volume:.2f} mm^3")
    ...
    # 循环 'i' 将是 0, 1, 2... (匹配 "0-5" 的文件夹)
    for i, class_name in enumerate(classes):
        df_new = np.zeros_like(df_volumes)
        
        # *** 已根据您的请求修改文件名 ***
        # 路径是 'sample_dir_path/cell_registration/0/cell_registration.csv'
        ...
        try:
            ...
            cell_ids = df_csv['id'].values
            count = np.bincount(cell_ids, minlength=len(indexes))
        except FileNotFoundError:
            print(f"警告: 文件未找到 {csv_path}. 将 {class_name} 计数设为 0。")
            count = np.zeros(len(indexes), dtype=int)
        except Exception as e:
        ...
        for j in range(1, len(ids)):
            current_id_path = f"/{ids[j]}/"
            idx = path.str.contains(current_id_path, na=False)
            count[j] = np.sum(count[idx])
        
        df_new = count.astype(float)
        num_list.append(df_new)
        
        density = np.divide(df_new, df_volumes, out=np.zeros_like(df_new), where=df_volumes!=0)
        density_list.append(density)

        table_count = pd.DataFrame(df_new, columns=[class_name])
        table_density = pd.DataFrame(density, columns=[f"{class_name}_density"])
